...
const Comments = ({ itemId, itemType }) => {
  ...
  const fetchComments = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:4000/api/comment/${itemType}/${itemId}`);
  ...
  const handleAddComment = async (e) => {
    ...
    try {
      const response = await axios.post(`http://localhost:4000/api/comment/${itemType}/${itemId}`, {
        content: newComment
      }, {
        headers: {
  ...
  const handleEditComment = async (commentId) => {
    ...
    try {
      const response = await axios.put(`http://localhost:4000/api/comment/${commentId}`, {
        content: editText
      }, {
        headers: {
          Authorization: `Bearer ${user.token}`
