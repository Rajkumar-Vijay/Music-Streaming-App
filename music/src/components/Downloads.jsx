...
const Downloads = () => {
  ...
  const { playTrack, track, isPlaying, togglePlay } = useContext(PlayerContext);
  ...
  const fetchDownloads = async () => {
    try {
      const response = await axios.get('http://localhost:4000/api/download/user', {
        headers: {
  ...
  const handleDownload = async (song) => {
    try {
      const response = await axios.get(`http://localhost:4000/api/download/song/${song._id}`, {
        headers: {
          Authorization: `Bearer ${user.token}`
        }
      });
