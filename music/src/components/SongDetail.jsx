...
const SongDetail = () => {
  ...
  const { playTrack, isPlaying, track, togglePlay, playlists } = useContext(PlayerContext);
  ...
  const fetchSong = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:4000/api/song/${id}`);
  ...
  const checkIfLiked = async () => {
    try {
      const response = await axios.get(`http://localhost:4000/api/like/check/song/${id}`, {
        headers: {
  ...
  const handleLike = async () => {
    ...
    try {
      if (liked) {
        await axios.delete(`http://localhost:4000/api/like/song/${id}`, {
          headers: {
            Authorization: `Bearer ${user.token}`
          }
        });
      } else {
        await axios.post(`http://localhost:4000/api/like/song/${id}`, {}, {
          headers: {
            Authorization: `Bearer ${user.token}`
          }
        });
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await axios.get(`http://localhost:4000/api/download/song/${id}`, {
        headers: {
          Authorization: `Bearer ${user.token}`
        }
      });
  ...
  const handleAddToPlaylist = async (playlistId) => {
    try {
      await axios.post(`http://localhost:4000/api/playlist/${playlistId}/songs/${id}`, {}, {
        headers: {
          Authorization: `Bearer ${user.token}`
        }
      });
